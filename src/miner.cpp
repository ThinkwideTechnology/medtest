...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("MedexcoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("MedexcoinMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("MedexcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("medexcoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) 
    {
        ...
        while (vNodes.empty() || IsInitialBlockDownload() || pwallet->IsLocked() ||  !fMintableCoins || nReserveBalance >= pwallet->GetBalance() || !masternodeSync.IsSynced())
        {
        ...
        LogPrintf("Running MedexcoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
