...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop Medexcoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "Medexcoin server stopping";
}
...
{ //  category              name                      actor (function)         okSafeMode threadSafe reqWallet
  //  --------------------- ------------------------  -----------------------  ---------- ---------- ---------
    ...
    /* Medexcoin features */
    { "medexcoin",               "masternode",             &masternode,             true,      true,       false },
    { "medexcoin",               "masternodelist",         &masternodelist,         true,      true,       false },
    { "medexcoin",               "mnbudget",               &mnbudget,               true,      true,       false },
    { "medexcoin",               "mnbudgetvoteraw",        &mnbudgetvoteraw,        true,      true,       false },
    { "medexcoin",               "mnfinalbudget",          &mnfinalbudget,          true,      true,       false },
    { "medexcoin",               "mnsync",                 &mnsync,                 true,      true,       false },
    { "medexcoin",               "spork",                  &spork,                  true,      true,       false },
#ifdef ENABLE_WALLET
    { "medexcoin",               "obfuscation",               &obfuscation,               false,     false,      true  }, /* not threadSafe because of SendMoney */
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("To use medexcoind, or the -server option to medexcoin-qt, you must set an rpcpassword in the configuration file:\n"
              "%s\n"
...
std::string HelpExampleCli(string methodname, string args){
    return "> medexcoin-cli " + methodname + " " + args + "\n";
}

std::string HelpExampleRpc(string methodname, string args){
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
        "\"method\": \"" + methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:11733/\n";
}
