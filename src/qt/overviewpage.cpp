...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate(): QAbstractItemDelegate(), unit(BitcoinUnits::MDEX)
    {
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    // update the display unit, to not use the default ("MDEX")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeMedexcoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeMedexcoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeMedexcoinAmount = strAnonymizeMedexcoinAmount.remove(strAnonymizeMedexcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMedexcoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeMedexcoinAmount*COIN) nMaxToAnonymize = nAnonymizeMedexcoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeMedexcoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeMedexcoinAmount));
        strAnonymizeMedexcoinAmount = strAnonymizeMedexcoinAmount.remove(strAnonymizeMedexcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMedexcoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::toggleObfuscation(){
    ...
    if(!fEnableObfuscation){
        ...
        if(nAnonymizeMedexcoinAmount == 0){
